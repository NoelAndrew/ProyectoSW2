...
{
    public class UpdateInfController : ApiController
    {
        public String Put([FromBody] UsuerInfo datos)
        {
                    ...
                    if (resp.Body.Trim('"') == strBuilder.ToString())
                    {
                        ...
                        if (resp.Body != "null")
                        {
                            ...
                            if (obj.rol == "rh")
                            {
                                ...
                                {
                                    client.Update("usuarios_info/" + datos.AuxUse, datos.JUser);
                                    resp = client.Get("respuestas/403");
                                    DateTime now = DateTime.Now;
                                    return JsonConvert.SerializeObject(new Respuesta() { Code = "403", Data = now.ToString("F"), Message = resp.Body, Status = "" });
                                }
                                else
                                {
